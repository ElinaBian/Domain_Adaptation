...
class MCD_DA():
    ...
    def generator(self, x, filter_size, n_filters_1, n_filters_2, n_units, reuse = False):
        # n_filters_1 = 32, n_filters_2 = 48
        # filter_size = 5
        ...
        with tf.variable_scope('generator', reuse = reuse):
            ...
            # conv1
            conv1 = tf.nn.conv2d(x_reshaped, w_1, strides = [1, 1, 1, 1], padding = 'VALID') + b_1
            ...
            # flatten
            conv_flat = tf.reshape(conv2, [-1, 4 * 4 * n_filters_2])   # 4*4*48
    ...
    def classifier_1(self, 
        ...
        with tf.variable_scope('classifier_1', reuse = reuse):
            ...
            # fc1
            fc1 = tf.matmul(x, w_1) + b_1

            # bn1_fc
            batch_mean_1, batch_var_1 = tf.nn.moments(fc1, [0])
            fc1 = (fc1 - batch_mean_1) / (tf.sqrt(batch_var_1) + 1e-5)
            ...
            # bn2_fc
            batch_mean_2, batch_var_2 = tf.nn.moments(fc2, [0])
            fc2 = (fc2 - batch_mean_2) / (tf.sqrt(batch_var_2) + 1e-5)
            ...
            w_3 = self.weight_variable('w_3', [n_units_3, 10])
            b_3 = self.bias_variable('b_3', [10])
            
            # fc3
            fc3 = tf.matmul(fc2, w_3) + b_3

            # bn3_fc
            batch_mean_3, batch_var_3 = tf.nn.moments(fc3, [0])
            fc3 = (fc3 - batch_mean_3) / (tf.sqrt(batch_var_3) + 1e-5)
    ...
    def classifier_2(self, 
                     x, 
                     n_units_1, # = n_units_g = 4*4*48
                     n_units_2, # 100
                     n_units_3, # = n_units_c = 100
                     keep_prob, # 0.5
                     reuse = False):
    ...
    def train(self, images_train_1, labels_train_1, images_test_1, labels_test_1, \
            ...
            filter_size, n_filters_1, n_filters_2, n_units_g, n_units_gc, n_units_c,\
        ...
        feat_1 = self.generator(x_1, filter_size, n_filters_1, n_filters_2, n_units_g, reuse = False)
        feat_t = self.generator(x_t, filter_size, n_filters_1, n_filters_2, n_units_g, reuse = True)

        logits_1_1 = self.classifier_1(feat_1, n_units_g, n_units_gc, n_units_c, keep_prob, reuse = False)
        probs_1_1 = tf.nn.softmax(logits_1_1)
        loss_1_1 = self.loss_cross_entropy(logits_1_1, y_1)

        logits_1_t = self.classifier_1(feat_t, n_units_g, n_units_gc, n_units_c, keep_prob, reuse = True)
        probs_1_t = tf.nn.softmax(logits_1_t)
        loss_1_t = self.loss_cross_entropy(logits_1_t, y_t)

        logits_2_1 = self.classifier_2(feat_1, n_units_g, n_units_gc, n_units_c, keep_prob, reuse = False)
        probs_2_1 = tf.nn.softmax(logits_2_1)
        loss_2_1 = self.loss_cross_entropy(logits_2_1, y_1)

        logits_2_t = self.classifier_2(feat_t, n_units_g, n_units_gc, n_units_c, keep_prob, reuse = True)
        ...
        best_model_name = 'MCD_Usps2Mnist_{}'.format(int(time.time()))

        with tf.Session() as sess:
            sess.run(init)
            writer = tf.summary.FileWriter("log/{}".format(best_model_name), sess.graph)
            saver = tf.train.Saver()
            ...
            for epc in range(epoch):
                ...
                for i in range(n_iter):
                    ...
                    # Train
                    # Step A
                    feed_A_dict = {x_1: x_train_batch, y_1: y_train_batch, keep_prob: 0.8} 
                    sess.run(train_step_a, feed_dict = feed_A_dict)

                    # Step B
                    feed_B_dict = {x_1: x_train_batch, y_1: y_train_batch, x_t: x_target_batch, keep_prob: 0.8}
                    sess.run(train_step_b, feed_dict = feed_B_dict)

                    # Step C
                    for n_c in range(num_stepC):
                        feed_C_dict = {x_t: x_target_batch, keep_prob: 0.75}
            ...
            saver.save(sess, 'usps_model/{}'.format(best_model_name))
            print("Best model saved. Model named {}".format(best_model_name))
